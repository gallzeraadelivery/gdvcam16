...
.method public static P(Landroid/content/SharedPreferences;I)V
    ...
    const-string p1, "MODE=$(cat /data/local/tmp/apexcam/recovery.mode 2>/dev/null); CUR=$(pidof cameraserver | awk '{print $1}'); OLD=$(awk -F'[ =]' '/^detail=pid=/{print $3}' /data/local/tmp/apexcam/daemon.status 2>/dev/null | head -1); if [ \"$MODE\" = restart_daemon ]; then if [ -n \"$CUR\" ] && { [ \"$CUR\" != \"$OLD\" ] || ! grep -q '^state=active' /data/local/tmp/apexcam/daemon.status 2>/dev/null || ! pidof apexcamd apexcamd.new >/dev/null 2>&1 || [ ! -p /data/local/tmp/apexcam/command.fifo ]; }; then PIDS=$(pidof apexcamd apexcamd.new 2>/dev/null); [ -z \"$PIDS\" ] || kill -9 $PIDS 2>/dev/null; rm -f /data/local/tmp/apexcam/command.fifo /data/local/tmp/apexcam/daemon.status; : > /data/local/tmp/apexcamd.log; setsid /data/local/tmp/apexcamd.new >/data/local/tmp/apexcamd.log 2>&1 < /dev/null & for I in $(seq 1 60); do [ -p /data/local/tmp/apexcam/command.fifo ] && break; sleep 0.1; done; timeout 8 sh -c 'echo start > /data/local/tmp/apexcam/command.fifo'; for I in $(seq 1 120); do grep -q '^state=active' /data/local/tmp/apexcam/daemon.status 2>/dev/null && break; grep -q '^state=error' /data/local/tmp/apexcam/daemon.status 2>/dev/null && break; sleep 0.1; done; fi; else if ! pidof apexcamd apexcamd.new >/dev/null 2>&1; then setsid /data/local/tmp/apexcamd.new >/data/local/tmp/apexcamd.log 2>&1 < /dev/null & for I in $(seq 1 40); do pidof apexcamd apexcamd.new >/dev/null 2>&1 && [ -p /data/local/tmp/apexcam/command.fifo ] && break; sleep 0.1; done; fi; if [ -z \"$CUR\" ] || [ \"$CUR\" != \"$OLD\" ] || ! grep -q '^state=active' /data/local/tmp/apexcam/daemon.status 2>/dev/null; then rm -f /data/local/tmp/apexcam/daemon.status; timeout 8 sh -c 'echo start > /data/local/tmp/apexcam/command.fifo'; for I in $(seq 1 120); do grep -q '^state=active' /data/local/tmp/apexcam/daemon.status 2>/dev/null && break; sleep 0.1; done; fi; fi; true"
